...
seq_out_hcp = tf.compat.v1.placeholder(tf.float32, [None, None, n_dim_hcp], 'seq_out_hcp')
seq_out_shape = tf.compat.v1.placeholder(tf.float32, [None, None, n_dim_shape], 'seq_out_shape')

w_hcp = tf.compat.v1.get_variable('w_hcp', shape=[lstm_cell.output_size, n_dim_hcp], dtype=tf.float32, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed=1337))
w_shape = tf.compat.v1.get_variable('w_shape', shape=[lstm_cell.output_size, n_dim_shape], dtype=tf.float32, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform", seed=1337))
...
cost_hcp = tf.compat.v1.losses.absolute_difference(out_hcp_target_seq, out_hcp_seq)
cost_shape = tf.compat.v1.losses.absolute_difference(out_shape_target_seq, out_shape_seq)
