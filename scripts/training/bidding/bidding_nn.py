...
# softmax_w = tf.compat.v1.get_variable('softmax_w', shape=[lstm_cell.output_size, n_bids],
#     dtype=tf.float32, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
#         mode="fan_avg", distribution="uniform", seed=1337))
...
print(X_train.shape, y_train.shape, y.shape)
...
model.add(keras.layers.Dense(8*40))
# categorical_crossentropy
model.compile(loss=keras.losses.SoftmaxLoss(),
    optimizer=keras.optimizers.Adam(1e-4),
    metrics=['accuracy'])

model.summary()

history = model.fit(X_train, y, validation_split=0.1,
    epochs=1, batch_size=16, steps_per_epoch=10000)
